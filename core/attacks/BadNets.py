...
class AddDatasetFolderTrigger(AddTrigger):
    """Add a trigger to DatasetFolder/ImageFolder images.

    The trigger should normally be inserted before ``ToTensor`` so that the
    input is a PIL image. Tensor inputs in CHW or HWC format are also handled.
    """
    ...
    def __init__(self, pattern, weight):
        ...
        if self.pattern.shape[-2:] != self.weight.shape[-2:]:
            raise ValueError(
                "pattern and weight must have the same spatial size. "
                f"Got {tuple(self.pattern.shape)} and {tuple(self.weight.shape)}."
            )

        self.res = self.weight * self.pattern
        self.weight = 1.0 - self.weight

    def _add_chw_tensor(self, image):
        if image.dim() == 2:
            image = image.unsqueeze(0)
            output = self.add_trigger(image).squeeze(0)
            return output

        if image.dim() != 3:
            raise ValueError(
                f"Unsupported tensor shape: {tuple(image.shape)}"
            )

        # CHW tensor.
        if image.shape[0] in (1, 3):
            return self.add_trigger(image)

        # HWC tensor.
        if image.shape[-1] in (1, 3):
            image_chw = image.permute(2, 0, 1)
            output = self.add_trigger(image_chw)
            return output.permute(1, 2, 0)

        raise ValueError(
            f"Cannot determine tensor channel dimension: {tuple(image.shape)}"
        )

    def __call__(self, img):
        if isinstance(img, PIL.Image.Image):
            tensor = F.pil_to_tensor(img)
            tensor = self._add_chw_tensor(tensor)

            if tensor.dim() == 2:
                return Image.fromarray(tensor.numpy(), mode="L")
            if tensor.size(0) == 1:
            ...
            raise ValueError(
                f"Unsupported PIL image tensor shape: {tuple(tensor.shape)}"
            )

        if isinstance(img, np.ndarray):
            tensor = torch.from_numpy(img)
            output = self._add_chw_tensor(tensor)
            return output.numpy()

        if isinstance(img, torch.Tensor):
            return self._add_chw_tensor(img)

        raise TypeError(
            "img should be PIL.Image.Image, numpy.ndarray, or torch.Tensor. "
            f"Got {type(img)}"
        )
...
class PoisonedDatasetFolder(DatasetFolder):
    """Poisoned DatasetFolder/ImageFolder.

    ``poisoned_set`` is optional. Supplying it allows the training,
    deterministic-analysis, and retraining views to share exactly the same
    ...
    def __init__(
        ...
        if poisoned_set is None:
            ...
            selected_indices = indices[:poisoned_num]
        else:
        ...
        self.poisoned_transform.transforms.insert(
            poisoned_transform_index,
            AddDatasetFolderTrigger(pattern, weight),
        )
        ...
        # Effective labels after poisoning. EPAR can use this field to build
        # the target-label subset without reading every image.
...
class PoisonedGTSRB(GTSRB):
    def __init__(self,
                 ...
                 poisoned_target_transform_index):
        # 初始化父类
        # 注意：GTSRB 内部通常用 _split 属性存储 'train' 或 'test'
        ...
        # 使用 AddDatasetFolderTrigger，因为它通用且支持 PIL/Tensor 处理
        # 这对于处理 GTSRB 的 resize -> trigger -> tensor 流程很合适
    ...
    def __getitem__(self, index):
        # GTSRB 在 torchvision 中使用 self._samples 存储 (path, target)
        path, target = self._samples[index]

        # 必须使用 PIL 加载并转为 RGB，以匹配 CIFAR10 的行为
...
def CreatePoisonedDataset(
    ...
    ImageFolder is a subclass of DatasetFolder, therefore ``isinstance`` must
    be used instead of ``type(dataset) == DatasetFolder``.
    """
    ...
    # ImageFolder inherits from DatasetFolder and enters this branch.
