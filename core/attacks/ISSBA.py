...
class PoisonedDatasetFolder(DatasetFolder):
    """Poisoned DatasetFolder/ImageFolder for Blended attack."""
...
class PoisonedGTSRB(GTSRB):
    def __init__(self,
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
        # 复用 AddDatasetFolderTrigger，因为它支持 PIL/Tensor 通用处理
...
def CreatePoisonedDataset(
    ...
    """Create Blended poisoned dataset.

    ImageFolder inherits from DatasetFolder, so isinstance must be used.
    """
